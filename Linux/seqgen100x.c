...
//    
//    Refernces: Tim Scherr, Sam Siewert:- https://github.com/siewertsmooc/RTES-ECEE-5623
//    Date: 9th April 2024
...
static volatile long wcet[7] = {0,0,0,0,0,0,0}; // Array to store WCET values
...
void main(void)
{
    ...
    printf("Starting Sequencer Demo RTES Exercise 5 Q3 seqgen100x.c\n\n");
   ...
   printf("\nTime Period and Deadline of the Sequencer and 7 services\n");
   
   printf("\nSequencer: \nTime Period: %lf msec, Deadline: %lf msec", (double)1000/3000, (double)1000/3000);      
   printf("\nFrame Sampler: \nTime Period: %lf msec, Deadline: %lf msec", (double)10/3, (double)10/3);
   printf("\nTime-stamp with Image Analysis: \nTime Period: %lf msec, Deadline: %lf msec", (double)10/1, (double)10/1);
   printf("\nDifference Image Proc: \nTime Period: %lf msec, Deadline: %lf msec", (double)2*10/1, (double)2*10/1);
   printf("\nTime-stamp Image Save to File: \nTime Period: %lf msec, Deadline: %lf msec", (double)10/1, (double)10/1);
   printf("\nProcessed Image Save to File: \nTime Period: %lf msec, Deadline: %lf msec", (double)2*10/1, (double)2*10/1);
   printf("\nSend Time-stamped Image to Remote: \nTime Period: %lf msec, Deadline: %lf msec", (double)10/1, (double)10/1);
   printf("\n10 Sec Tick Debug: \nTime Period: %lf msec, Deadline: %lf msec\n\n", (double)10*10/1, (double)10*10/1);
   ...
   printf("\nWCET for 7 services\n");
   printf("\nFrame Sampler WCET: %lu usec", wcet[0]);
   printf("\nTime-stamp with Image Analysis WCET: %lu usec", wcet[1]);
   printf("\nDifference Image Proc WCET: %lu usec", wcet[2]);
   printf("\nTime-stamp Image Save to File WCET: %lu usec", wcet[3]);
   printf("\nProcessed Image Save to File WCET: %lu usec", wcet[4]);
   printf("\nSend Time-stamped Image to Remote WCET: %lu usec", wcet[5]);
   printf("\n10 Sec Tick Debug WCET: %lu usec\n", wcet[6]);
...
{
    struct timeval current_time_val;
    struct timespec delay_time = {0,333333}; // delay for 00.33 msec, 3000 Hz
...
void *Service_1(void *threadp)
{
    ...
    {
        if(sem_wait(&semS1) == 0) {
          // Gets the time as soon as the semphore is released
          gettimeofday(&current_time_val, (struct timezone *)0); 
          syslog(LOG_CRIT, "Frame Sampler release llu @ sec=%d, msec=%d\n", (int)(current_time_val.tv_sec-start_time_val.tv_sec), (int)current_time_val.tv_usec/USEC_PER_MSEC);
        }
        clock_gettime(CLOCK_MONOTONIC, &start_time);
        S1Cnt++;
        FIB_TEST(seqIterations, fibCnt);  // Fixed load
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        exec_time = (long)end_time.tv_nsec/1000 - (long)start_time.tv_nsec/1000;
        if (exec_time > wcet[0]) { wcet[0] = exec_time; }
        syslog(LOG_CRIT, "Frame Sampler %llu : execution time: msec=%lu\n", S1Cnt, exec_time);
    }
...
void *Service_2(void *threadp)
{
    ...
    while(!abortS2)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        exec_time = (long)end_time.tv_nsec/1000 - (long)start_time.tv_nsec/1000;
        if (exec_time > wcet[1]) { wcet[1] = exec_time; }
        syslog(LOG_CRIT, "Time-stamp with Image Analysis %llu : execution time: msec=%lu\n", S2Cnt, exec_time);
    }
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        exec_time = (long)end_time.tv_nsec/1000 - (long)start_time.tv_nsec/1000;
        if (exec_time > wcet[2]) { wcet[2] = exec_time; }
        syslog(LOG_CRIT, "Difference Image Proc %llu : execution time: msec=%lu\n", S3Cnt, exec_time);
    }
...
void *Service_4(void *threadp)
{
    ...
    while(!abortS4)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        exec_time = (long)end_time.tv_nsec/1000 - (long)start_time.tv_nsec/1000;
        if (exec_time > wcet[3]) { wcet[3] = exec_time; }
        syslog(LOG_CRIT, "Time-stamp Image Save to File %llu : execution time: msec=%lu\n", S4Cnt, exec_time);
    }
...
void *Service_5(void *threadp)
{
    ...
    while(!abortS5)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        exec_time = (long)end_time.tv_nsec/1000 - (long)start_time.tv_nsec/1000;
        if (exec_time > wcet[4]) { wcet[4] = exec_time; }
        syslog(LOG_CRIT, "Processed Image Save to File %llu : execution time: msec=%lu\n", S5Cnt, exec_time);
    }
...
void *Service_6(void *threadp)
{
    ...
    while(!abortS6)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        exec_time = (long)end_time.tv_nsec/1000 - (long)start_time.tv_nsec/1000;
        if (exec_time > wcet[5]) { wcet[5] = exec_time; }
        syslog(LOG_CRIT, "Send Time-stamped Image to Remote %llu : execution time: msec=%lu\n", S6Cnt, exec_time);
    }
...
void *Service_7(void *threadp)
{
    ...
    while(!abortS7)
    {
        ...
        clock_gettime(CLOCK_MONOTONIC, &end_time);
        exec_time = (long)end_time.tv_nsec/1000 - (long)start_time.tv_nsec/1000;
        if (exec_time > wcet[6]) { wcet[6] = exec_time; }
        syslog(LOG_CRIT, "10 Sec Tick Debug %llu : execution time: msec=%lu\n", S7Cnt, exec_time);
    }
