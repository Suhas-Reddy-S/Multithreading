...
uint32_t wcet[7] = {0, 0, 0, 0, 0, 0, 0}, T[8] = {10000/30, 10000/3, 10000, 20000, 10000, 20000, 10000, 100000};
...
void Timer0IntHandler(void)
{
    ...
    if (g_tick % 1 == 0) {
        xSemaphoreGive(semSched); // releases sequencer semaphore at 3000Hz
    }
...
static void Sequencer_thread(void *params)
{
    ...
    int i = 0;
    UARTprintf("Sequencer Thread: Time Period = %u usec\n", T[i]);
    for (i = 1; i < 8; i++) {
        UARTprintf("Service %d: Time Period = %u usec, Deadline = %u usec, WCET = %u msec\n", i, T[i], T[i], wcet[i-1]);
    }
    UARTprintf("\n");
}
...
int main(void)
{
    ...
    ConfigureUART();
    UARTprintf("\nRTES Exercise 5 Q3 - seqgen100x.c\n\n");
...
// Restrict services to start before sequencer
    ...
    ROM_TimerLoadSet(TIMER0_BASE, TIMER_A, ROM_SysCtlClockGet()/3000);   // Configure timer0A to run at 3000HZ
    ...
    xTaskCreate(Sequencer_thread, "Sequencer Thread", 128, NULL, tskIDLE_PRIORITY + 6, NULL);
    xTaskCreate(service1, "Service 1", 128, NULL, tskIDLE_PRIORITY + 5, NULL);
    xTaskCreate(service2, "Service 2", 128, NULL, tskIDLE_PRIORITY + 4, NULL);
    xTaskCreate(service3, "Service 3", 128, NULL, tskIDLE_PRIORITY + 3, NULL);
    ...
    xTaskCreate(service6, "Service 6", 128, NULL, tskIDLE_PRIORITY + 4, NULL);
    xTaskCreate(service7, "Service 7", 128, NULL, tskIDLE_PRIORITY + 2, NULL);
