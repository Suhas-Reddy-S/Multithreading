...
uint32_t wcet[7] = {0, 0, 0, 0, 0, 0, 0}, T[8] = {1000/60, 1000/30, 100, 200, 100, 200, 100, 1000};
...
// Runs @ 60Hz
static void Scheduler_thread(void *params)
{
    uint32_t startTime;
    static volatile uint32_t schedCnt = 0;
    startTime = xTaskGetTickCount();
    UARTprintf("Scheduler Started at %u msec\n", startTime);
    while (schedCnt < 1800) {
...
// Restrict services to start before sequencer
    ...
    xTaskCreate(Scheduler_thread, "Scheduler Thread", 128, NULL, tskIDLE_PRIORITY + 5, NULL);
